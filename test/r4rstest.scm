...
(use-module (test))
(define primitive-test test)       
(define-macro (test expect fun . args)
`(let ((test-passed? (if (procedure? ,fun)
        (primitive-test ,expect (,fun ,@args))
        (primitive-test ,expect ,@args))))
   (if test-passed?
       '()
       (set! errs (cons (cons ',fun ',args) errs)))))


(define report-errs test-end)
(define (report-errs)
  (test-end)
  (newline)
